...
import { Plus, Building2, Wallet, Trash2, Radio, Edit2 } from 'lucide-react';
...
const BANK_SUGGESTIONS = ['Nubank', 'Itaú', 'Bradesco', 'Banco do Brasil', 'Caixa', 'Santander', 'Inter', 'C6 Bank', 'XP'];
...
export const Accounts: React.FC = () => {
  ...
  const handleSave = async () => {
    if (!formData.institution.trim() || !formData.balance) {
    ...
    try {
      ...
      if (editingId) {
        const { error } = await supabase.from('accounts').update(accData).eq('id', editingId);
        ...
        if (error) throw error;
        addToast('Conta adicionada!', 'success');
      }
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Remover conta permanentemente?')) return;
    ...
    <div className="space-y-8 max-w-7xl mx-auto pb-24">
      <div className="flex justify-between items-center">
        ...
        <Button
          ...
          className="h-12 bg-primary-600 hover:bg-primary-700 shadow-xl shadow-primary-500/20 px-8"
        >
      ...
      {accounts.length > 0 && (
        <div className="bg-gradient-to-r from-primary-600 to-primary-800 rounded-[2.5rem] p-8 text-white shadow-2xl shadow-primary-500/20">
          <p className="text-[10px] font-black uppercase tracking-[0.25em] text-primary-200">Saldo Total</p>
          <p className="text-4xl font-black mt-2">
            {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(totalBalance)}
          </p>
          <p className="text-primary-300 text-xs mt-1 font-medium">{accounts.length} conta{accounts.length !== 1 ? 's' : ''} ativa{accounts.length !== 1 ? 's' : ''}</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {accounts.map(acc => (
          <Card key={acc.id} className="group relative bg-white dark:bg-gray-800 border-none shadow-sm hover:shadow-xl transition-all p-8 rounded-[2.5rem] overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-primary-500/5 rounded-full -mr-10 -mt-10 group-hover:scale-110 transition-transform" />
            ...
            <div className="flex items-center justify-between mb-8 relative z-10">
              ...
              <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => handleOpenEdit(acc)} className="p-2 text-gray-400 hover:text-primary-600 transition-colors">
                  <Edit2 size={16} />
                </button>
                <button onClick={() => handleDelete(acc.id)} className="p-2 text-gray-300 hover:text-rose-500 transition-colors">
                  <Trash2 size={18} />
                </button>
              </div>
            </div>
            ...
            <div className="mt-10 pt-6 border-t border-gray-50 dark:border-gray-700 relative z-10">
              ...
              <p className={`text-3xl font-black ${acc.balance < 0 ? 'text-rose-600' : 'text-gray-900 dark:text-white'}`}>
      ...
      <Modal
        ...
        title={editingId ? 'Editar Conta' : 'Conectar Instituição'}
      >
        <div className="space-y-6">
          <div>
            ...
            />
            <datalist id="banks-list">{BANK_SUGGESTIONS.map(b => <option key={b} value={b} />)}</datalist>
          </div>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Saldo Atual (R$)</label>
              <input
                type="number"
                step="0.01"
                className="w-full bg-gray-50 dark:bg-gray-900 border-none rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary-500 outline-none"
          ...
          <Button fullWidth size="lg" onClick={handleSave} loading={saving} className="py-4 shadow-xl shadow-primary-500/20">
            {editingId ? 'Salvar Alterações' : 'Confirmar Conexão'}
          </Button>
        </div>
      </Modal>
    </div>
